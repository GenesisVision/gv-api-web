...
import { LoginSignMessage } from '../model/LoginSignMessage';
import { LoginSignViewModel } from '../model/LoginSignViewModel';
...
export default class AuthApi {
  ...
  authorizeBySignature = (
    ...
    return generateMethod<Promise<string>>({
        init,
        apiClient: this.apiClient,
        path: "/v2.0/auth/signature/signin",
        body: JSON.stringify(options['body']),
        returnType: "primitive",
        method: "POST",
    })
  ...
  getMsgForAuthorizeBySignature = (
    options: {
      } = {},
    init: RequestInit = {}): Promise<LoginSignMessage> => {
    
    return generateMethod<Promise<LoginSignMessage>>({
        init,
        apiClient: this.apiClient,
        path: "/v2.0/auth/signature/message",
