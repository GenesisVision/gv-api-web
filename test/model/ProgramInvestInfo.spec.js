/**
 * Core API v1.0
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: v1.0
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 *
 * Swagger Codegen version: 2.3.1
 *
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', '../../src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require('../../src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.CoreApiV10);
  }
}(this, function(expect, CoreApiV10) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new CoreApiV10.ProgramInvestInfo();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('ProgramInvestInfo', function() {
    it('should create an instance of ProgramInvestInfo', function() {
      // uncomment below and update the code to test ProgramInvestInfo
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be.a(CoreApiV10.ProgramInvestInfo);
    });

    it('should have the property periodEnds (base name: "periodEnds")', function() {
      // uncomment below and update the code to test the property periodEnds
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

    it('should have the property availableToInvest (base name: "availableToInvest")', function() {
      // uncomment below and update the code to test the property availableToInvest
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

    it('should have the property availableToInvestBase (base name: "availableToInvestBase")', function() {
      // uncomment below and update the code to test the property availableToInvestBase
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

    it('should have the property title (base name: "title")', function() {
      // uncomment below and update the code to test the property title
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

    it('should have the property availableInWallet (base name: "availableInWallet")', function() {
      // uncomment below and update the code to test the property availableInWallet
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

    it('should have the property minInvestmentAmount (base name: "minInvestmentAmount")', function() {
      // uncomment below and update the code to test the property minInvestmentAmount
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

    it('should have the property entryFee (base name: "entryFee")', function() {
      // uncomment below and update the code to test the property entryFee
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

    it('should have the property gvCommission (base name: "gvCommission")', function() {
      // uncomment below and update the code to test the property gvCommission
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

    it('should have the property rate (base name: "rate")', function() {
      // uncomment below and update the code to test the property rate
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

    it('should have the property isOwnProgram (base name: "isOwnProgram")', function() {
      // uncomment below and update the code to test the property isOwnProgram
      //var instance = new CoreApiV10.ProgramInvestInfo();
      //expect(instance).to.be();
    });

  });

}));
